...
export default function AccountItem({ account }: Props): JSX.Element {
  ...
  return (
    <li className="relative">
      <div
        className={`group flex items-center gap-2.5 rounded-md pl-2 pr-1 py-1.5 transition-colors ${
      ...
      >
        ...
        <button
          ...
          className="flex h-6 w-5 items-center justify-center rounded text-muted opacity-0 transition hover:bg-black/10 group-hover:opacity-100"
          aria-label="更多操作"
        >
          ⋯
        </button>
      ...
      {menuOpen && (
        <>
          ...
          />
          <div className="absolute right-2 top-11 z-40 w-40 overflow-hidden rounded-md border border-border bg-white shadow-lg">
            <button
              type="button"
              onClick={onUpdate}
              className="block w-full px-3 py-1.5 text-left text-xs hover:bg-sidebar"
            >
              更新应用密码
            </button>
            <button
              ...
              className="block w-full px-3 py-1.5 text-left text-xs text-danger hover:bg-red-50"
            >
              移除账号
            </button>
          </div>
